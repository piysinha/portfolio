...
- **Strategy pattern.** Settling up is a `SettleUpStrategy`, so the algorithm can change without touching the rest of the code. The heap-based strategy nets each person's balance, then pairs the largest amount owed with the largest amount due.
